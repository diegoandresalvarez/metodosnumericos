...
def print_results(x, term, exp):
    ...
    print("\n")
    print("Let's add positive terms first and then negative terms to see if it makes a difference in the final result.")
    new_exp = np.sum(term[::2]) + np.sum(term[1::2])  # Sum positive and negative terms separately
    print(f"Sum of positive terms = {np.sum(term[::2]):.10g}")
    print(f"Sum of negative terms = {np.sum(term[1::2]):.10g}")
    print(f"New sum = {new_exp:.10g}")
    print(f"Absolute error in new sum = {np.abs(exact_value-new_exp):.10f}")
    print(f"Relative error in new sum = {100*np.abs(exact_value-new_exp)/exact_value:.10f}%")
